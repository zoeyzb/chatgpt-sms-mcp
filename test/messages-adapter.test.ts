...
class FakeRunner implements CommandRunner {
  ...
  constructor(private readonly responder: (command: string, args: string[]) => Promise<{ stdout: string; stderr: string }>) {}
...
describe('MessagesAdapter account discovery', () => {
  it('marks sending available when one account is SMS even if other accounts fail service type lookup', async () => {
    ...
    expect(status.details.join('\n')).toContain('SMS account: 5CCEB943-86A6-48E3-B263-C008B7D9D6DE');
  });
  ...
  it('sends through the discovered SMS account using participant syntax', async () => {
    ...
    const runner = new FakeRunner(async (command, args) => {
      if (command !== 'osascript') throw new Error(`unexpected command: ${command}`);
      osascriptCalls++;
      if (osascriptCalls === 1) {
        return {
          stdout: '5CCEB943-86A6-48E3-B263-C008B7D9D6DE\tSMS\tmissing value\n',
          stderr: ''
        };
      }
      const script = args.at(-1) ?? '';
      expect(script).toContain('account id "5CCEB943-86A6-48E3-B263-C008B7D9D6DE"');
      expect(script).toContain('participant "+13125551234" of targetAccount');
      expect(script).toContain('send "hello" to targetParticipant');
